...
public class OrderApplication {
	...
	public CommandLineRunner run(RestTemplate restTemplate,StreamBridge streamBridge) throws Exception {
		return args -> {
			...
			String[] quantity = new String[]{"7","8","10","16","22","55","9","1"};
			String[] product = new String[]{"Monitor","Microphone","Wireless Charger","Rug","Desk Chair"};
				...
				try {
					...
					while (!Thread.currentThread().isInterrupted()){
						...
						Order orders = restTemplate.postForObject("http://localhost:8081/order/"+ids[rndId]+"/"+product[rndProduct]+"/"+quantity[rndQuantity],Order.class.toString(),Order.class);
