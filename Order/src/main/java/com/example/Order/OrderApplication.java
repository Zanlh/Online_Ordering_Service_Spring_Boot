			...
			String[] product = new String[]{"Monitor","Microphone","Wireless Charger","Rug","Desk Chair"};
				...
				try {
					...
					while (!Thread.currentThread().isInterrupted()){
						...
						Product oneProduct = restTemplate.getForObject("http://localhost:8082/products/productName"+"?productName="+product[rndProduct], Product.class);
						Order orders = restTemplate.postForObject("http://localhost:8081/order/"+ids[rndId]+"/"+product[rndProduct]+"/1/"+oneProduct.getPrice(),Order.class.toString(),Order.class);
